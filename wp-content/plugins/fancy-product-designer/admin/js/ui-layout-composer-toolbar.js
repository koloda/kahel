jQuery(document).ready(function($) {
	...
	*********************************/
	...
	function addGtStep(targetVal, textVal, $gtStep) {
		...
		var $lastItem = $gtStep.append('<div class="fpd-gt-item"><div class="fpd-gt-item-actions"><span class="fpd-drag dashicons dashicons-move"></span><span class="fpd-gt-item-remove fpd-admin-icon-bin"></span></div><select><option></option></select><textarea placeholder="Enter explanation text - HTML tags are supported."></textarea></div>').children('.fpd-gt-item:last');
...
function fpd_get_gt_step_json($stepContainer) {
	...
	$stepContainer.children('div').each(function(i, step) {
		...
		settingsObj[$step.children('select').val()] = $step.children('textarea').val();
